...
// Was gross auf dem Plakat steht: die Zahl aus "7 Krimiserien ...", sonst ein
// Zeichen fuer die Artikelform.
function glyphFor(post) {
  const zahl = String(post.meta.title || '').match(/^(\d{1,2})\s/);
  if (zahl) return { t: 'z', w: zahl[1] };
  if (post.meta.format === 'vergleich') return { t: 'z', w: 'VS' };
  if (post.meta.format === 'guide') return { t: 'i', w: 'buch' };
  if (post.meta.format === 'wo-streamen') return { t: 'i', w: 'play' };
  return { t: 'i', w: 'ticket' };
}
